        ...
        try:
            ...
            <a href='""" + item['webViewLink'] \
                    ...
                    if title in created_pages:
                        # If the title already exists, append a number to make it unique.
                        num = 1
                        while title in created_pages:
                            num += 1
                            title = f"{itemname}_{num}"
                    ...
                    get_child_sub_folders(service, current_id, level, Html_file, folders_only, folder_id, drivetype, parent_page_id)
...
def get_child_sub_folders(service, parent_id, level, Html_file, folders_only, folder_id, drivetype, parent_page_id):
            ...
            if items:
                for item in items:
                    ...
                    if title in created_pages:
                        ...
                        while title in created_pages:
                            num += 1
                            title = f"{childname}_{num}"
                    created_pages.append(title)
                    
                    soup = BeautifulSoup(html_list3, 'html.parser')
                    body_value = str(soup)
                    new_parent_page_id = create_confluence_page(title, parent_page_id, body_value)
                    get_child_sub_folders(service, child_id, level, Html_file, folders_only, folder_id, drivetype, new_parent_page_id)
                page_token = results.get('nextPageToken')
                Html_file.write("""
...
def main():
    ...
    parent_id = create_confluence_page("KB catalog", PARENT_ID, "https://drive.google.com/drive/u/0/folders/0ANbyjco9413kUk9PVA")
